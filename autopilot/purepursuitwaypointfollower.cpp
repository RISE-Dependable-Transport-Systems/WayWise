...
double PurepursuitWaypointFollower::getCurvatureWithTrailer(QSharedPointer<VehicleState> vehicleState, const QPointF &point, PosType vehiclePosType)
{
    const PosPoint vehiclePos = vehicleState->getPosition(vehiclePosType);
    double currYaw_rad = vehiclePos.getYaw() * M_PI / 180.0;
    double measuredTrailerAngle = vehicleState->getTrailerAngleRadians() ;
    ...
    if (vehicleState->getSpeed()> 0 ){
        ...
        double desired_hitch_angle = atan(2*mCurrentState.trailerWheelBase*sin(theta_err)); // desired trailer/hitch angle
        return  mCurrentState.forwardGainWithTrailer*( measuredTrailerAngle - desired_hitch_angle ) - ( sin(measuredTrailerAngle)/ (mCurrentState.trailerWheelBase)) ;

    } else {
        double trailerYaw = currYaw_rad - measuredTrailerAngle ;
        double delta_x_ = (mCurrentState.trailerWheelBase) * cos( trailerYaw);
        double delta_y = (mCurrentState.trailerWheelBase) * sin( trailerYaw);
        ...
        double desired_hitch_angle = atan(2*mCurrentState.trailerWheelBase*sin(theta_err) / vehicleState->getAutopilotRadius() );
        return  mCurrentState.reverseGainWithTrailer*( measuredTrailerAngle - desired_hitch_angle ) - ( sin(measuredTrailerAngle)/ (mCurrentState.trailerWheelBase)) ;
    }
}
...
double PurepursuitWaypointFollower::getCurvatureToPointInENU(const QPointF &point)
{
    ...
    if (vehicleState->getHasTrailer()) {
        return getCurvatureWithTrailer(vehicleState, point, mPosTypeUsed);
    } else {
