...
double PurepursuitWaypointFollower::getCurvatureToPointInENU(QSharedPointer<VehicleState> vehicleState, const QPointF &point, PosType vehiclePosType)
{
    ...
    if (vehicleState->getSpeed()>0 ){
        ...
        double theta_err =  atan2(pointInTrailerFrame.y(), pointInTrailerFrame.x()) - trailerAngle; // the Theta between the target point and curect position 
...
// qDebug()<< "start1 " << currentVehiclePositionXY; 
