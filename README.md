...
```bash
docker run --rm risedas/waywise:latest /usr/local/bin/RCCar_ISO22133_autopilot
```

3. **map_local_twocars** - Two-car local simulation

```bash
docker run --rm risedas/waywise:latest /usr/local/bin/map_local_twocars
```
...
2. ControlTower should automatically connect to `udp://:14550`
