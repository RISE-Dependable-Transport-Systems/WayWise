    ...
    : GNSSReceiver(vehicleState)
{
    ...
    connect(&mUblox, &Ublox::rxEsfAlg, this, [this](const ubx_esf_alg &alg){
        ...
        if (mPrintVerbose && mUbloxState == UBLOX_STATE::CONFIGURED){
            qDebug() << "---------------------------------";
            qDebug() << "ESF-ALG data:"
                     << "\nStatus:" << alg.status
    ...
    if (mPrintVerbose) {
        ...
        connect(&mUblox, &Ublox::rxEsfStatus, this, [this](const ubx_esf_status &status) {
            if (mUbloxState == UBLOX_STATE::CONFIGURED){
                qDebug() << "---------------------------------";
...
bool UbloxRover::connectSerial(const QSerialPortInfo &serialPortInfo)
{
    if (!mUblox.connectSerial(serialPortInfo)) {
        mUbloxState = UBLOX_STATE::DISCONNECTED;
        return false; // Failed to connect to the serial port
    }

    mUbloxState = UBLOX_STATE::CONNECTED;

    mUblox.ubxCfgRst(0, 0x09); // start receiver
    qDebug() << "UbloxRover: Receiver start request was sent";
    ...
    if (mUbloxState != UBLOX_STATE::BACKUP_RESTORED) {
        mUblox.ubxCfgRst(0x01, 0x01); // request software reset with warm start to remove previous restore request and continue
    }

    // Configure the receiver
    if (!configureUblox()) {
        qDebug() << "Failed to configure Ublox receiver.";
        mUbloxState = UBLOX_STATE::DISCONNECTED;
...
bool UbloxRover::configureUblox()
{
    ...
    if (mUbloxVariant == UBLOX_VARIANT::ZED_F9R) {
        // F9R specific configuration
        mUblox.ubloxCfgAppendE1U1Key(buffer, &ind, CFG_NAVSPG_DYNMODEL, mDynamicModel); // Set Dynmaic model
        int cfg_rate_meas = 1000 / mGNSSMeasurementRate; // convert Hz to ms
        mUblox.ubloxCfgAppendRate(buffer, &ind, cfg_rate_meas, 1, 0, mNavPrioMessageRate); // Enable/disable nav prio mode

        if (mUbloxState != UBLOX_STATE::BACKUP_RESTORED || mForceRecalibrateSensors) {
        ...
        if (mNavPrioMessageRate > 0) {
            mUblox.ubloxCfgAppendE1U1Key(buffer, &ind, CFG_MSGOUT_UBX_NAV_PVT_USB, mNavPrioMessageRate); // UBX-NAV-PVT rate
        } else {
            mUblox.ubloxCfgAppendE1U1Key(buffer, &ind, CFG_MSGOUT_UBX_NAV_PVT_USB, mGNSSMeasurementRate); // UBX-NAV-PVT rate
        }

        result &= mUblox.ubloxCfgValset(buffer, ind, true, true, true, 0); // timeout set to 0 as ack is not being received
    ...
    } else if (mUbloxVariant == UBLOX_VARIANT::ZED_F9P) {
    ...
    mUbloxState = UBLOX_STATE::CONFIGURED;
    return true;
}
...
void UbloxRover::restoreBackedupConfiguration(int pollIntervalms, int maxPolls)
{
    ...
    if (restore_bkup_response == 2) {
        mUbloxState = UBLOX_STATE::BACKUP_RESTORED;
    }
}
...
void UbloxRover::createConfigurationBackup(int pollIntervalms, int maxPolls)
{
    ...
    if (create_bkup_response == 2) {
        mUbloxState = UBLOX_STATE::BACKUP_CREATED;
    }
}
...
void UbloxRover::saveOnShutdown()
{
    if (mUblox.isSerialConnected()) {
        switch (mUbloxState)
        {
            case UBLOX_STATE::DISCONNECTED:
            case UBLOX_STATE::UNKNOWN:
            case UBLOX_STATE::CONNECTED:
            case UBLOX_STATE::BACKUP_RESTORED:
                break;
            default: {

                // Disable nav prio mode
                unsigned char buffer[4096];
                int ind = 0;
                mUblox.ubloxCfgAppendRate(buffer, &ind, 1000, 1, 0, 0);
                bool result = mUblox.ubloxCfgValset(buffer, ind, true, true, true, 0); // timeout set to 0 as ack is not being received
                qDebug() << "UbloxRover: Nav prio mode" << (result ? "was turned off" : "could not be turned off");
                ...
                result = mUblox.ubxCfgRst(0, 0x08);
                qDebug() << "UbloxRover: Receiver stop request" << (result ? "was sent" : "reported an error");

                if (mUbloxState == UBLOX_STATE::CONFIGURED) {
                    mUbloxState = UBLOX_STATE::SHUTTING_DOWN; // suppress logs
